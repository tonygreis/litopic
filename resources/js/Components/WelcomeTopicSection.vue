<template>
    <div class="p-6 bg-gray-100 dark:bg-gray-900 rounded-lg">
        ...
        <div class="grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-4">
            <div v-for="item in data" :key="item" class="group flex flex-col overflow-hidden space-x-4 items-center bg-white dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-large p-2">
                <Link :href="item.url"
                ...
                <div>
                    <Link :href="item.url" class="m-4 flex text-center">
                    <h3 class="sm:text-lg text-base font-semibold text-slate-900 dark:text-white leading-tight group-hover:text-slate-800 dark:group-hover:text-slate-200">
                        {{ item.name }} </h3>
                    </Link>
                </div>
            </div>
